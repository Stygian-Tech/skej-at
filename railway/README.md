...
Railway is the canonical web and gateway host for Development and Production.
GitHub Actions validates source changes; Railway deploys the linked branch
through its GitHub integration.

Both services keep the monorepo root directory at `/`. `Gateway` uses
`/railway/gateway.json`, while `Web` uses `/railway/web.json`. Development
tracks `dev`; Production tracks `main`.
